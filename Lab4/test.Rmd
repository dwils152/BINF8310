...
After removing the lowly expressed genes, the number of p-values that equal 1 is significantly reduced.
...
poisson.test(data[1,2], sum(data[, 2] ,r=p))
...
plot_p_vals_4 <- data.frame(log10(p_vals_3), log10(p_vals))

ggplot(data=plot_p_vals_4, aes(x=p_vals, y=p_vals_3)) + geom_point(alpha=0.4) + xlab("Log10(Poisson P-values)") + ylab("Log10(P-values)") + theme(text= element_text(size=14), axis.title.x=element_text(face="bold"),  axis.title.y=element_text(face="bold"))
